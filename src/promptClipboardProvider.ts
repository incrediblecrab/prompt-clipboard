...
export class PromptClipboardProvider implements vscode.TreeDataProvider<SectionItem>, vscode.TreeDragAndDropController<SectionItem> {
    dropMimeTypes = ['application/vnd.code.tree.promptclipboard'];
    dragMimeTypes = ['application/vnd.code.tree.promptclipboard'];
    ...
    private lastBaseName: string = 'template';
    private fileCounter: number = 1;
    private preferredFormat: 'txt' | 'md' = 'txt';
    private txtFileCounter: number = 1;
    private mdFileCounter: number = 1;
    ...
    async handleDrag(source: SectionItem[], treeDataTransfer: vscode.DataTransfer): Promise<void> {
        treeDataTransfer.set('application/vnd.code.tree.promptclipboard', new vscode.DataTransferItem(source));
    }
    
    async handleDrop(target: SectionItem | undefined, sources: vscode.DataTransfer): Promise<void> {
        const transferItem = sources.get('application/vnd.code.tree.promptclipboard');
        if (!transferItem) {
            return;
        }
        
        const draggedItems: SectionItem[] = transferItem.value;
    ...
    private loadSections(): void {
        ...
        const savedSections = this.context.workspaceState.get<Section[]>(`${workspaceKey}.promptClipboardSections`);
        const savedBaseName = this.context.workspaceState.get<string>(`${workspaceKey}.lastBaseName`);
        const savedCounter = this.context.workspaceState.get<number>(`${workspaceKey}.fileCounter`);
        const savedPreferredFormat = this.context.workspaceState.get<'txt' | 'md'>(`${workspaceKey}.preferredFormat`);
        const savedTxtCounter = this.context.workspaceState.get<number>(`${workspaceKey}.txtFileCounter`);
        const savedMdCounter = this.context.workspaceState.get<number>(`${workspaceKey}.mdFileCounter`);
    ...
    removeSection(item: SectionItem): void {
    ...
    async editSection(item: SectionItem): Promise<void> {
        const section = item.section;
        this.openSectionEditor(section, false);
    }

    private openSectionEditor(section: Section, isNew: boolean = false): void {
        const panel = vscode.window.createWebviewPanel(
            'sectionEditor',
            isNew ? 'Add New Section' : `Edit: ${section.title}`,
    ...
    private getSectionEditorHtml(section: Section, isNew: boolean = false): string {
        return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<body>
    ...
    duplicateSection(item: SectionItem): void {
        const section = item.section;
        const newSection: Section = {
            id: Date.now().toString(),
    ...
    async generateTemplate(): Promise<void> {
        try {
            ...
            const selectedFormat: 'txt' | 'md' = formatChoice.label.includes('.txt') ? 'txt' : 'md';
